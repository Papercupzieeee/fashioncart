...
const ORDERS_BACKEND_CANDIDATES = [BACKEND_BASE, "http://localhost:5001"];
let resolvedOrdersBackendBase = null;
...
async function resolveOrdersBackendBase(userId = getCurrentUserId()) {
  if (resolvedOrdersBackendBase) return resolvedOrdersBackendBase;

  for (const base of ORDERS_BACKEND_CANDIDATES) {
    try {
      const res = await fetch(`${base}/api/orders/${userId}`);
      if (res.status !== 404) {
        resolvedOrdersBackendBase = base;
        return resolvedOrdersBackendBase;
      }
    } catch (_error) {
      // Try the next candidate backend.
    }
  }

  resolvedOrdersBackendBase = BACKEND_BASE;
  return resolvedOrdersBackendBase;
}
...
async function clearCartAfterOrder(cartItems) {
  ...
  await Promise.all(
    removableItems.map(async (item) => {
      try {
        await fetch(`${BACKEND_BASE}/api/cart/remove/${item.cartItemId}`, { method: "DELETE" });
      } catch (_error) {
        // Ignore cleanup failures and still clear the in-memory cart.
      }
    })
  );
...
function renderOrders(container, orders) {
  ...
  container.innerHTML = orders.map(order => {
    ...
    const trackingAction = status === "Out for Delivery"
      ? `<a href="track-order.html?orderId=${order.id}" class="btn btn-secondary btn-small" style="margin-top: 1rem; display: inline-flex;">Track Order</a>`
      : `<button class="btn btn-secondary btn-small" style="margin-top: 1rem; opacity: 0.55; cursor: not-allowed;" disabled>Track Order</button>`;
      ...
      return `
        <div style="display: flex; gap: 1rem; margin-bottom: 1rem; align-items: center; border-bottom: 1px rgba(0,0,0,0.05) solid; padding-bottom: 0.5rem;">
          ...
          <div>
            ...
            <p style="font-size: 0.9rem; color: var(--text-light);">Qty: ${quantity} | â‚¹${unitPrice}</p>
          </div>
        </div>
      `;
    }).join("");
      ...
      <div class="order-card" style="border: 1px solid var(--border-color); border-radius: 12px; padding: 1.5rem; background: var(--bg-white); margin-bottom: 2rem; box-shadow: var(--shadow-sm);">
        <div style="display: flex; justify-content: space-between; border-bottom: 1px solid var(--border-color); padding-bottom: 1rem; margin-bottom: 1rem;">
          ...
          <div style="text-align: right;">
            ...
            <p style="font-weight: 800; font-size: 1.1rem; margin-top: 8px;">â‚¹${orderTotal}</p>
          </div>
        </div>
        <div>${itemsHtml}</div>
        <div>${trackingAction}</div>
      </div>
    `;
  }).join('');
}
...
async function placeOrder() {
  ...
  try {
    ...
    if (res.ok) {
      await clearCartAfterOrder(cartItems);
      showToast('Order placed successfully!', 'success');
      setTimeout(() => { window.location.href = 'orders.html'; }, 1500);
    } else {
      const errorData = await res.json().catch(() => ({}));
      const fallbackMessage = res.status === 404
        ? 'Orders API not found. Restart the backend server.'
        : 'Failed to place order';
      showToast(errorData.message || fallbackMessage, 'error');
    }
  } catch (e) {
...
// Final order overrides placed at end-of-file so they take precedence.
placeOrder = async function() {
  const cartItems = getCart();
  if (cartItems.length === 0) return showToast('Your cart is empty', 'warning');

  const userId = getCurrentUserId();
  const ordersBase = await resolveOrdersBackendBase(userId);
  const orderItems = cartItems.map(item => ({
    productId: item.id,
    price: item.price,
    quantity: item.quantity
  ...
  try {
    const res = await fetch(`${ordersBase}/api/orders`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ userId, items: orderItems })
    });
...
loadOrdersPage = async function() {
  ...
  try {
    const userId = getCurrentUserId();
    const ordersBase = await resolveOrdersBackendBase(userId);
    const res = await fetch(`${ordersBase}/api/orders/${userId}`);
    if (!res.ok) {
      if (res.status === 404) {
        container.innerHTML = '<p>Orders API not found. Restart the backend server.</p>';
        return;
      }
